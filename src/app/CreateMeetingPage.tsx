...
function DescriptionInput({ value, onChange }: DescriptionInputProps) {
  ...
  return (
    <div className="spave-y-2">
      <div className="font-medium">Metting info:</div>
...
function StartTimeInput({ value, onChange }: StartTimeInputProps) {
  ...
  return (
    <div className="space-y-2">
      <div className="font-medium">Metting start:</div>
      <label className="flex items-center gap-1.5">
        <input
          type="radio"
          checked={!active}
      ...
      {active && (
        <label className="block space-y-1">
          ...
          <textarea
            value={value}
            onChange={(e) => onChange(e.target.value)}
            placeholder="enter participants email addresses seperated by commas"
            className="w-full rounded-md border border-gray-300 p-2"
          />
        </label>
      )}
...
function getMailToLink(
  ...
  const startDateFormatted = startTime
  ...
  const body =
    `Join the meeting at ${meetingLink}` +
    (startDateFormatted ? `\n\n at ${startDateFormatted}.` : "") +
    (description ? `\n\n ${description}` : "");
